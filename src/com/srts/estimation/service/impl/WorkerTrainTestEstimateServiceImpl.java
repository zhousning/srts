...
@Service
public class WorkerTrainTestEstimateServiceImpl implements WorkerTrainTestEstimateService {
	@Resource
	private WorkerTrainTestEstimateDao dao;
	@Resource
	...
	public String findUserTestScoreRankByUserAndType(Sys_User usr) {
		...
		while(iterator.hasNext()){
			...
			String testName=dao.findTestNameByTrainAndType("正式", testPaperId);
	...
	public List<UserTestEstimateInfoPo> setUserTestEstimateInfo(Sys_User usr) {
		...
		List<Integer> resList0=dao.categoryUserTestScore(usr, "正式");
		List resList1=dao.findUserTestScoreByUserAndType(usr, "正式");
		...
		if(testNum!=0)
		{
			...
			if(scoreBelowLine>0)
			{
				estimateString+="调考合格率为"+String.valueOf(passRate*100)+"%.";
				estimateString+="历次调考中存在考试不合格的情况,"+"共"+String.valueOf(scoreBelowLine)+"次.";
		...
		{
			UserTestEstimateInfoPo temp=new UserTestEstimateInfoPo();
			temp.setEstimateString("暂时没有进行过调考,无评价");
			temp.setType("考试成绩评估");
			temp.setId("1");
			res.add(temp);
		}
