...
<html>
  <head>
	...
	$(function(){
		...
		$("#page").change(function(){
			window.location="info/InformationUploadAction_downloadNoticeByPage.action?page="+$("#page").val();
		});
	});
</script>
	<style type="text/css">
  ...
  <div class="panel_main">
	...
	<s:iterator value="pageBean.list" status="st">
		...
		<s:else>
			<a
				href="info/InformationUploadAction_downloadNoticeByPage.action?page=<s:property value="%{pageBean.currentPage-1}"/>">上一页</a>
		</s:else>
		<s:if test="%{pageBean.currentPage != pageBean.totalPage}">
			<a
				href="info/InformationUploadAction_downloadNoticeByPage.action?page=<s:property value="%{pageBean.currentPage+1}"/>">下一页</a>
		</s:if>
		<s:else>下一页 </s:else>
		跳转到第
		<select name="" id="page">
